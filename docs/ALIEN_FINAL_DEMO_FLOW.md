# Alien Final Demo Flow

## 1. Overview

The `dark-alien-final-demo` crate implements a 10-step end-to-end demo pipeline
that ties together every ALIEN_FINAL_BOSS primitive in a single local / mock
...
The output schema is described in [Section 5](#5-output--distalien-finaldemo_runjson).
...
```bash
cargo test -p dark-alien-final-demo -- --nocapture
```
...
fn main() {
    let run = dark_alien_final_demo::run_demo();
    let json = serde_json::to_string_pretty(&run).unwrap();
    std::fs::write("dist/alien-final/DEMO_RUN.json", json).unwrap();
...
```bash
cargo test -p dark-alien-final-demo test_demo_run_completes -- --nocapture 2>&1
```

---

## 5. Output — `dist/alien-final/DEMO_RUN.json`
...
A placeholder file lives at `dist/alien-final/DEMO_RUN.json`. Run the crate to
