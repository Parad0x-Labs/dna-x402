...
- 84 tests passed
- 0 tests failed
- 80 receipts built + chained + batched off-chain; **0 anchored on-chain** (corrected 2026-06-05 — no anchor TX exists for this run; the report's own Audit Summary shows `receiptsAnchored: 0`, and the `receipt_anchor` program had no on-chain activity until 2026-05-29, ~3 months after this run)
- 20 on-chain USDC transfer txs (genuine, finalized on mainnet)
