...
| 2 | Third-party audit sign-off | Not started | dist/alien-final/evidence/audit_signed.json |
...
| 6 | No forbidden claim failures (check-alien-final-claims.mjs) | Script ready | — |
| 7 | Signed deploy plan with program IDs | Not created | dist/alien-final/evidence/signed_deploy_plan.json |
...
| 12 | No HMAC-lite in production path | Fixed in Phase 9 | dist/alien-final/evidence/hmac_rfc2104.json |
| 13 | x402 devnet tx verified (if x402 claimed) | Mock only | dist/alien-final/evidence/x402_devnet_real.json |
| 14 | Poseidon real backend tested (if Poseidon claimed) | Blocked | dist/alien-final/evidence/poseidon_real.json |
| 15 | ZK proof real backend wired (if ZK claimed) | Mock only | dist/alien-final/evidence/zk_verifier_real.json |
| 16 | Compression real integration (if compression claimed) | Simulator only | dist/alien-final/evidence/zk_compression_real.json |

## Command to Run Gate Check

```bash
node scripts/check-mainnet-alien-final.mjs
```
