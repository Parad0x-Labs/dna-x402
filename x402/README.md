...
That is the fastest scaffold, not the strongest control surface. `dnaSeller()` now verifies `transfer` proofs through the local Solana payment verifier, emits real signed receipts for the payment finalize handshake, and for unlocked JSON routes emits a second delivery-bound receipt tied to the actual protected response body. Unlocked text responses now also carry a signed delivery receipt in the `x-dna-receipt` header. Binary/manual streaming handlers still fall back to the finalize-handshake receipt. Failed `5xx` deliveries do not burn the paid unlock; the scaffold restores the commit for retry instead of pretending a broken response was delivered. Guard policies, replay controls, market routing, and anchoring still live in the full x402 server path.
