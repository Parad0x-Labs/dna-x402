...
export class ReplayStore {
  ...
  constructor(private readonly ttlMs = 24 * 60 * 60 * 1000) {
    const hasDurable = !!(process.env.POSTGRES_URL || process.env.DATABASE_URL);
    if (!hasDurable) {
      if (process.env.NODE_ENV === "production") {
        throw new Error(
          "ReplayStore: durable replay protection required in production. Set POSTGRES_URL or DATABASE_URL. In-memory replay allows double-spend attacks across restarts."
        );
      }
      console.warn(
        "WARNING: ReplayStore is in-memory. Replay attacks are possible across restarts. Configure POSTGRES_URL or DATABASE_URL for durable replay protection."
      );
    }
  }
   ...
   * store (POSTGRES_URL or DATABASE_URL) has been configured.
   */
