...
pub enum SourceChain { Solana = 1, EthereumL2 = 2, Arbitrum = 3 }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum DestChain { Ethereum = 1, Arbitrum = 2, Optimism = 3 }
...
pub enum CrossChainError { SameSourceAndDest, InvalidNullifier }

pub fn create_cross_chain_receipt(
    source_chain: SourceChain, dest_chain: DestChain,
    program_id_bytes: &[u8], instruction_data: &[u8],
    slot: u64, nullifier: &[u8; 32], evm_target_hash: &[u8; 32],
    ...
    let same = matches!(
        ...
        (SourceChain::EthereumL2, DestChain::Arbitrum) | (SourceChain::Arbitrum, DestChain::Arbitrum)
    );
    if same { return Err(CrossChainError::SameSourceAndDest); }
    if nullifier == &[0u8; 32] { return Err(CrossChainError::InvalidNullifier); }
    ...
    let bridge_hash = sha256_chain("xchain-bridge-v1", &[&[source_chain as u8], &[dest_chain as u8], nullifier, &slot_le]);
    let evm_calldata_hash = sha256_chain("evm-calldata-v1", &[&bridge_hash, evm_target_hash]);
    Ok(CrossChainReceipt {
        source_chain, dest_chain,
        bridge_hash,
        solana_proof: SolanaProofSummary { program_id_hash, instruction_hash, slot, nullifier: *nullifier },
        evm_calldata_hash,
        mainnet_ready: false,
    })
}
...
pub fn verify_cross_chain_receipt(receipt: &CrossChainReceipt) -> bool {
    receipt.bridge_hash != [0u8; 32] && receipt.evm_calldata_hash != [0u8; 32] && receipt.solana_proof.nullifier != [0u8; 32]
}
...
mod tests {
    ...
    fn nullifier(b: u8) -> [u8; 32] { let mut n = [b; 32]; n[0] = 1; n }
    fn target() -> [u8; 32] { [0xAB; 32] }

    #[test]
    fn test_create_cross_chain_receipt_happy_path() {
        let r = create_cross_chain_receipt(SourceChain::Solana, DestChain::Ethereum, b"prog", b"ix", 100_000, &nullifier(5), &target()).unwrap();
    ...
    fn test_same_source_dest_rejected() {
        ...
        let err = create_cross_chain_receipt(SourceChain::EthereumL2, DestChain::Arbitrum, b"p", b"i", 1, &nullifier(1), &target()).unwrap_err();
        assert_eq!(err, CrossChainError::SameSourceAndDest);
    }

    #[test]
    fn test_zero_nullifier_rejected() {
        let err = create_cross_chain_receipt(SourceChain::Solana, DestChain::Ethereum, b"p", b"i", 1, &[0u8; 32], &target()).unwrap_err();
        assert_eq!(err, CrossChainError::InvalidNullifier);
    }

    #[test]
    fn test_bridge_hash_deterministic() {
        let a = create_cross_chain_receipt(SourceChain::Solana, DestChain::Ethereum, b"p", b"i", 999, &nullifier(7), &target()).unwrap();
        let b = create_cross_chain_receipt(SourceChain::Solana, DestChain::Ethereum, b"p", b"i", 999, &nullifier(7), &target()).unwrap();
        assert_eq!(a.bridge_hash, b.bridge_hash);
    }

    #[test]
    fn test_public_record_hides_instruction() {
        let r = create_cross_chain_receipt(SourceChain::Solana, DestChain::Optimism, b"prog_id_bytes", b"secret_instruction_data", 42, &nullifier(3), &target()).unwrap();
        let json = receipt_public_record(&r);
        let ix_hex = "secret_instruction_data".as_bytes().iter().map(|x| format!("{:02x}", x)).collect::<String>();
