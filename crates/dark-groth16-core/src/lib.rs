...
pub const G2_GEN_X_IM: [u8; 32] =
    hex_const("198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2");
/// G2 generator x — real part.
pub const G2_GEN_X_RE: [u8; 32] =
    hex_const("1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed");
/// G2 generator y — imaginary part.
pub const G2_GEN_Y_IM: [u8; 32] =
    hex_const("090689d0585ff075ec9e99ad690c3395bc4b313370b38ef355acdadcd122975b");
/// G2 generator y — real part.
pub const G2_GEN_Y_RE: [u8; 32] =
    hex_const("12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa");

/// G2 generator negation y — imaginary part.
/// Verified from Ethereum EIP-197 "two_point_match_2" test vector.
pub const G2_GEN_NEG_Y_IM: [u8; 32] =
    hex_const("275dc4a288d1afb3cbb1ac09187524c7db36395df7be3b99e673b13a075a65ec");
/// G2 generator negation y — real part.
pub const G2_GEN_NEG_Y_RE: [u8; 32] =
    hex_const("1d9befcd05a5323e6da4d435f3b617cdb3af83285c2df711ef39c01571827f9d");
...
pub fn g1_mul_scalar(point: &G1Affine, scalar: &[u8; 32]) -> Result<G1Affine, Groth16Error> {
    ...
    let arr: [u8; 64] = out
        .try_into()
        .map_err(|_| Groth16Error::Bn254Error(AltBn128Error::UnexpectedError))?;
...
pub fn g1_generator_neg() -> G1Affine {
    G1Affine {
        x: G1_GEN_X,
        y: G1_GEN_NEG_Y,
    }
...
pub fn g2_generator_neg() -> G2Affine {
    G2Affine {
        x_im: G2_GEN_X_IM,
        x_re: G2_GEN_X_RE,
        y_im: G2_GEN_NEG_Y_IM,
        y_re: G2_GEN_NEG_Y_RE,
    }
}
...
mod tests {
    ...
    fn test_proof_byte_layout() {
        let mut raw = [0u8; 256];
        raw[0] = 0xA0;
        raw[32] = 0xA1; // A.x[0], A.y[0]
        raw[64] = 0xB0;
        raw[96] = 0xB1; // B.x_im[0], B.x_re[0]
        raw[128] = 0xB2;
        raw[160] = 0xB3; // B.y_im[0], B.y_re[0]
        raw[192] = 0xC0;
        raw[224] = 0xC1; // C.x[0], C.y[0]
        let p = proof_from_bytes(&raw);
        assert_eq!(p.a.x[0], 0xA0);
        assert_eq!(p.a.y[0], 0xA1);
        assert_eq!(p.b.x_im[0], 0xB0);
        assert_eq!(p.b.x_re[0], 0xB1);
        assert_eq!(p.b.y_im[0], 0xB2);
        assert_eq!(p.b.y_re[0], 0xB3);
        assert_eq!(p.c.x[0], 0xC0);
        assert_eq!(p.c.y[0], 0xC1);
    }
    ...
    fn test_negate_g1_double_identity() {
        let g1 = g1_generator();
        assert_eq!(
            negate_g1(&negate_g1(&g1)),
            g1,
            "double negate must be identity"
        );
    }
