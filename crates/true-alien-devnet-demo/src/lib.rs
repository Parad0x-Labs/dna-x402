// true-alien-devnet-demo/src/lib.rs
//
// NOT_PRODUCTION — devnet only, mainnet_ready=false
//
// Builds all 10 True Alien Primitive objects locally and returns a typed
...
pub fn build_local_demo() -> DemoEvidenceLocal {
    ...
    let capsule_no_custody = NoCustodyCapsule {
        binary_hash: sha256_bytes(b"true_alien_demo_binary_v1"),
        config_hash: sha256_bytes(b"true_alien_demo_config_v1"),
        denied_key_classes: DeniedKeyClass::all(),
        max_float_lamports: 0,
        redaction_policy_hash: sha256_bytes(b"pii_removed"),
        custody_denied: true,
        issued_at_slot: slot,
        signer_pubkey_hash: sha256_bytes(b"true_alien_demo_signer"),
    };
    ...
    // ── Roadmap commitment ────────────────────────────────────────────────────
    let docs_hash = sha256_bytes(b"TRUE_ALIEN_PRIMITIVES.md_v1");
    let tests_hash = sha256_bytes(b"true_alien_tests_522_v1");
    let roadmap_commit = commit_feature(
        docs_hash,
        tests_hash,
        b"Dark Null True Alien Primitives -- 10 product primitives, 522 tests",
        0,            // target_epoch
