// true_frontier_devnet_demo — True Frontier Primitives Devnet Demo
//
// NOT_PRODUCTION — devnet only, mainnet_ready=false
//
// What this binary does:
//   1-8. Runs all 10 True Frontier Primitives locally (no network needed).
...
//   10.  Writes dist/true-frontier/TRUE_FRONTIER_DEVNET_DEMO.json
//   11.  Writes docs/TRUE_FRONTIER_DEVNET_DEMO_EVIDENCE.md
//
// Usage:
//   cargo run -p true-frontier-devnet-demo --bin true_frontier_devnet_demo
//
// Optional env vars:
...
use std::str::FromStr;
use true_frontier_devnet_demo::{build_local_demo, hex_encode};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("╔═════════════════════════════════════════════════════╗");
    println!("║   Dark Null True Frontier Primitives — Devnet Demo     ║");
    ...
    std::fs::create_dir_all("dist/true-frontier")?;
    let json_path = "dist/true-frontier/TRUE_FRONTIER_DEVNET_DEMO.json";
    ...
    let md_path = "docs/TRUE_FRONTIER_DEVNET_DEMO_EVIDENCE.md";
    std::fs::write(md_path, &md)?;
    println!("  Written: {}", md_path);

    // ── Summary ───────────────────────────────────────────────────────────────
    println!("\n╔═════════════════════════════════════════════════════╗");
    println!("║  True Frontier Demo Complete                            ║");
...
fn build_evidence_md(
    local: &true_frontier_devnet_demo::DemoEvidenceLocal,
    shards: &[ShardEntry],
    txs: &[String],
    links: &[String],
    live_mode: bool,
    ...
    format!(
        r#"# Dark Null True Frontier Primitives — Devnet Demo Evidence
