...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RentGoblinSweepPlan {
    pub targets: Vec<RentGoblinTarget>,
...
pub fn scan_mock_targets(current_slot: u64) -> Vec<RentGoblinTarget> {
...
pub fn build_sweep_plan(targets: Vec<RentGoblinTarget>, current_slot: u64) -> RentGoblinSweepPlan {
    let eligible: Vec<RentGoblinTarget> = targets
        .into_iter()
        .filter(|t| {
...
pub fn sort_by_highest_bounty(targets: &mut Vec<RentGoblinTarget>) {
