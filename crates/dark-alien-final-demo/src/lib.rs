...
/// dark-alien-final-demo — Phase 8 ALIEN_FINAL_BOSS demo pipeline.
///
/// Ties together every ALIEN_FINAL_BOSS primitive in a single local / mock run:
...
/// Full demo run output — written to dist/alien-final/DEMO_RUN.json.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DemoRun {
...
fn make_requirement() -> X402PaymentRequirement {
    X402PaymentRequirement {
        ...
        pay_to: [0xAA; 32],
        resource: "https://api.darknull.example/alien-final".to_string(),
...
/// Execute the full 10-step alien-final demo pipeline and return a `DemoRun`.
///
...
pub fn run_demo() -> DemoRun {
    ...
    let payer = [0xBBu8; 32];
    let proof = make_proof(&req, payer, "MOCK_SIG_alienfinalboss_v1_deadbeef");
    ...
    let receipt =
        mint_receipt_note_after_payment(&req, &proof, b"alien_final_response_payload", 1_000)
    ...
    let public_inputs = b"alien_final_public_inputs_v1";
    ...
    let claim = ProofClaim {
        ...
        domain: b"dark_alien_final_demo".to_vec(),
    };
    ...
    DemoRun {
        ...
        public_summary: "Alien-final scaffolding complete. Local/mock proof layer tested. \
