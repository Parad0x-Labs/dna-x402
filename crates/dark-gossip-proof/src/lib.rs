...
pub enum GossipError {
    EmptyMessage,
    TooManyHops { max: u8, got: u8 },
    ReceiverMismatch,
}

pub const MAX_HOPS: u8 = 7;

pub fn create_gossip_receipt(
    message_bytes: &[u8],
    sender_nonce: &[u8; 32],
    receiver_secret: &[u8; 32],
    received_at_unix: i64,
    hop_count: u8,
) -> Result<GossipReceipt, GossipError> {
    if message_bytes.is_empty() {
        return Err(GossipError::EmptyMessage);
    }
    if hop_count > MAX_HOPS {
        return Err(GossipError::TooManyHops {
            max: MAX_HOPS,
            got: hop_count,
        });
    }
    ...
    let receiver_commitment =
        sha256_chain("gossip-recv-v1", &[receiver_secret, &message_commitment]);
    Ok(GossipReceipt {
        message_commitment,
        receiver_commitment,
        received_at_unix,
...
pub fn prove_receipt(
    receipt: &GossipReceipt,
    receiver_secret: &[u8; 32],
) -> Result<GossipProof, GossipError> {
    let recomputed = sha256_chain(
        "gossip-recv-v1",
        &[receiver_secret, &receipt.message_commitment],
    );
    if recomputed != receipt.receiver_commitment {
        return Err(GossipError::ReceiverMismatch);
    }
    let ts = receipt.received_at_unix.to_le_bytes();
    let proof_hash = sha256_chain(
        "gossip-proof-v1",
        &[
            &receipt.message_commitment,
            &receipt.receiver_commitment,
            &ts,
        ],
    );
    Ok(GossipProof {
        proof_hash,
        message_commitment: receipt.message_commitment,
...
mod tests {
    ...
    fn test_empty_message_rejected() {
        assert_eq!(
            create_gossip_receipt(b"", &make_nonce(1), &make_nonce(2), 1700000000, 3).unwrap_err(),
            GossipError::EmptyMessage
        );
    }
    ...
    fn test_too_many_hops_rejected() {
        assert_eq!(
            create_gossip_receipt(b"msg", &make_nonce(1), &make_nonce(2), 1700000000, 8)
                .unwrap_err(),
            GossipError::TooManyHops { max: 7, got: 8 }
        );
    }
    ...
    fn test_wrong_receiver_rejected() {
        let r = create_gossip_receipt(b"signal", &make_nonce(1), &make_nonce(42), 1700000000, 2)
            .unwrap();
        assert_eq!(
            prove_receipt(&r, &make_nonce(99)).unwrap_err(),
