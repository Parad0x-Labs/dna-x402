/**
 * deploy-frontier-tek.ts
 *
 * Deploys Dark Null Solana-Native Frontier Research programs to devnet.
 * Records program IDs back into docs/SOLANA_FRONTIER_RESEARCH.md.
 *
 * Usage:
...
const FRONTIER_DOC  = join(ROOT, "docs", "SOLANA_FRONTIER_RESEARCH.md");
...
function updateDocProgramId(programName: string, programId: string): void {
  let doc = readFileSync(FRONTIER_DOC, "utf-8");
  doc = doc.replace(
    new RegExp(`(\\| ${programName}\\s+\\| devnet\\s+\\|)\\s+_pending deployment_`),
    `$1 \`${programId}\``
  );
  writeFileSync(FRONTIER_DOC, doc, "utf-8");
}

// ── Main ──────────────────────────────────────────────────────────────────────

async function main(): Promise<void> {
  console.log(`\n🚀 Dark Null Frontier Research — deploying to ${CLUSTER} (${RPC_URL})\n`);
  ...
  console.log("\ndocs/SOLANA_FRONTIER_RESEARCH.md updated with program IDs.");
  console.log("Commit with: git add docs/SOLANA_FRONTIER_RESEARCH.md && git commit -m 'feat: devnet frontier research program IDs'");
}
