#!/usr/bin/env node
/**
 * check-mainnet-frontier-final.mjs
...
const EVIDENCE_DIR = path.join(BASE, "dist/frontier-final/evidence");
...
console.log("=== Dark Null Mainnet Frontier Final Gate ===\n");
...
check(
  ...
  audit !== null,
  "Missing: dist/frontier-final/evidence/audit_signed.json"
);
...
try {
  execSync("node scripts/check-frontier-final-claims.mjs", { encoding: "utf8", timeout: 30000 });
  claimCheckPasses = true;
} catch { claimCheckPasses = false; }
check("check-frontier-final-claims.mjs passes", claimCheckPasses, "Run: node scripts/check-frontier-final-claims.mjs");
...
check(
  "Signed deploy plan",
  deployPlan !== null,
  "Missing: dist/frontier-final/evidence/signed_deploy_plan.json"
);
...
check(
  ...
  hmacEvidence !== null,
  "Missing: dist/frontier-final/evidence/hmac_rfc2104.json"
);
