 ...
 * deploy-alien-tek.ts
 *
 * Builds and deploys the three core Dark Null Alien Tek on-chain programs:
 *   - dark_nullifier_banks
 *   - dark_compressed_receipts
 *   - dark_chaff
 *
 * Usage:
 *   npx tsx scripts/deploy-alien-tek.ts [options]
 *
 * Options:
 ...
 *                                                (default: scripts/deploy/alien-tek-program-ids.json)
 *
 * After a successful deploy the program IDs are written to:
 *   scripts/deploy/alien-tek-program-ids.json
 *
 * and docs/SOLANA_ALIEN_TEK.md is updated with the live devnet IDs.
 */
...
function parseArgs(argv: string[]): Record<string, string | boolean> {
  const out: Record<string, string | boolean> = {
    ...
    "dry-run": false,
    "skip-build": false,
    out: path.join(ROOT, "scripts/deploy/alien-tek-program-ids.json"),
  };
...
if (!SKIP_BUILD) {
  console.log("\n── Building Alien Tek programs (cargo build-sbf) ──");
  for (const prog of ALIEN_PROGRAMS) {
...
for (const prog of ALIEN_PROGRAMS) {
  ...
  const existingId = programIdFromKeypair(path.join(ROOT, "target/deploy", prog.soName));
...
// ── Step 4: Patch SOLANA_ALIEN_TEK.md with live IDs ──────────────────────────

if (!DRY_RUN) {
  const docPath = path.join(ROOT, "docs/SOLANA_ALIEN_TEK.md");
  if (fs.existsSync(docPath)) {
    ...
    console.log("── docs/SOLANA_ALIEN_TEK.md patched with program IDs ──");
  }
}
...
console.log("\n═══════════════════════════════════════");
console.log("  Dark Null Alien Tek — Deploy Summary");
console.log("═══════════════════════════════════════");
